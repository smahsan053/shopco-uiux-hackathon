...
function Feedback() {
  return (
    <>
      <h1 className="font-bold text-5xl text-wrap md:text-nowrap text-center mt-8">
        OUR HAPPY CUSTOMERS
      </h1>
      <Carousel
        opts={{
        ...
        className="relative w-full max-w-7xl mx-auto flex justify-center items-center"
      >
        {/* Navigation Arrows */}
        <div className="absolute top-10 right-16 flex gap-2 z-10">
          <CarouselPrevious className="bg-gray-300 rounded-full p-2 hover:bg-gray-400" />
          <CarouselNext className="bg-gray-300 rounded-full p-2 hover:bg-gray-400" />
        </div>

        {/* Carousel Content */}
        <CarouselContent className="flex gap-4 px-0 mt-16 sm:px-0 lg:px-0 justify-center">
          {customerResponse.map((response, index) => (
            <CarouselItem
              key={index}
              className="flex justify-center items-start flex-shrink-0 w-full max-w-md"
            >
